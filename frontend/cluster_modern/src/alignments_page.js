...
async function apiGet(path) {
  const r = await fetch(path, { headers: getToken() ? { Authorization: `Bearer ${getToken()}` } : {} });
  if (!r.ok) throw new Error(`HTTP ${r.status}: ${await r.text()}`);
  return r.json();
}
...
function statusPill(status) {
  ...
  return `<span style="display:inline-block; background:${c.bg}; color:${c.color}; font-size:11px; font-weight:600; padding:3px 10px; border-radius:999px; letter-spacing:0.04em;">${htmlEscape(s)}</span>`;
}
...
function renderPassageBlock(label, accentBg, accentColor, accentBorder, passage) {
  ...
  const ctxToggle = (targetId, labelText) =>
    `<button type="button" onclick="(function(el){el.style.display=el.style.display==='none'?'block':'none'})(document.getElementById('${targetId}'))"
  ...
  return `
    <div style="display:flex; flex-direction:column; border:1.5px solid ${accentBorder}; border-radius:8px; overflow:hidden; min-width:0;">
      ...
      <div style="background:${accentBg}; color:${accentColor}; font-size:12px; font-weight:600; padding:6px 12px; display:flex; justify-content:space-between; align-items:center; letter-spacing:0.03em; flex-shrink:0;">
        ...
        <span style="font-weight:400; font-size:10px; opacity:0.75;">passage_id ${htmlEscape(passage.passage_id ?? "—")}</span>
      </div>
      ...
      ${hasBefore ? `
        ...
        <div id="${beforeId}" style="display:none; background:#f3f3f3; border-top:1px solid #e0e0e0; padding:8px 12px; flex-shrink:0;">
          <pre style="margin:0; font-size:11px; color:#666; white-space:pre-wrap; font-family:monospace; line-height:1.5;">${htmlEscape(passage.context_before)}</pre>
        </div>
      ` : ""}
      ...
      <div style="background:#ffffff; border-top:1px solid #e0e0e0; padding:12px; flex:1;">
        ${content
          ? `<pre style="margin:0; font-size:13px; color:#1a1a1a; white-space:pre-wrap; font-family:monospace; line-height:1.65;">${htmlEscape(content)}</pre>`
      ...
      ${hasAfter ? `
        ...
        <div id="${afterId}" style="display:none; background:#f3f3f3; border-top:1px solid #e0e0e0; padding:8px 12px; flex-shrink:0;">
          <pre style="margin:0; font-size:11px; color:#666; white-space:pre-wrap; font-family:monospace; line-height:1.5;">${htmlEscape(passage.context_after)}</pre>
        </div>
      ` : ""}
...
const ANNOT_STATUSES = ["UNREVIEWED", "OUI", "NON", "DOUTEUX", "DISCARDED"];
 
function renderAnnotEditor(alignmentId, currentStatus, currentComment) {
  const st = (currentStatus || "UNREVIEWED").toUpperCase();
  const edId  = `ed-${alignmentId}`;
  const msgId = `msg-${alignmentId}`;
  return `
    <div id="${edId}" style="display:none; padding:14px 16px; background:#f7f7f7; border-top:1px solid #e0e0e0;">
      <div style="display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap;">
        <div>
          <div style="font-size:11px; color:#888; margin-bottom:4px; font-weight:500;">Statut</div>
          <select id="sel-${alignmentId}" style="padding:6px 10px; border:1px solid #ccc; border-radius:6px; font-size:13px; background:#fff; color:#1a1a1a;">
            ${ANNOT_STATUSES.map(s => `<option value="${s}"${s === st ? " selected" : ""}>${s}</option>`).join("")}
          </select>
        </div>
        <div style="flex:1; min-width:180px;">
          <div style="font-size:11px; color:#888; margin-bottom:4px; font-weight:500;">Commentaire</div>
          <input id="com-${alignmentId}" type="text" value="${htmlEscape(currentComment || "")}" placeholder="(optionnel)"
            style="width:100%; padding:6px 10px; border:1px solid #ccc; border-radius:6px; font-size:13px; background:#fff; color:#1a1a1a; box-sizing:border-box;" />
        </div>
        <button
          onclick="saveAnnot(${alignmentId})"
          style="padding:7px 16px; border:1px solid #aaa; border-radius:6px; background:#fff; font-size:13px; cursor:pointer; white-space:nowrap;">
          Enregistrer
        </button>
      </div>
      <div id="${msgId}" style="font-size:12px; color:#888; margin-top:6px; min-height:16px;"></div>
    </div>
  `;
}
  ...
  return `
    <div style="border:2px solid #d0d0d0; border-radius:10px; margin-bottom:20px; overflow:hidden; background:#fff; box-shadow:0 1px 4px rgba(0,0,0,0.06);">
      ...
      <div style="display:flex; align-items:center; gap:10px; padding:12px 16px; background:#f5f5f5; border-bottom:1px solid #e0e0e0; flex-wrap:wrap;">
        ...
        <span style="font-size:15px; font-weight:700; color:#1a1a1a; letter-spacing:-0.01em;">#${htmlEscape(aid)}</span>
        <div style="flex:1;"></div>
        <div id="pill-${aid}">${statusPill(status)}</div>
        <button
          onclick="toggleEditor(${aid})"
          style="padding:6px 14px; border:1px solid #999; border-radius:6px; background:#fff; font-size:13px; cursor:pointer; font-weight:500; color:#1a1a1a;">
          Annoter
        </button>
      </div>
 
      <!-- ── EDITEUR D'ANNOTATION ── -->
      ${renderAnnotEditor(aid, status, r.latest_comment ?? "")}
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; border-bottom:1px solid #e0e0e0;">
        <div style="padding:10px 16px; border-right:1px solid #e0e0e0;">
          <div style="font-size:13px; font-weight:600; color:#1a1a1a; margin-bottom:2px;">${htmlEscape(r.source_title || "—")}</div>
          <div style="font-size:12px; color:#555;">${htmlEscape(r.source_authors || "")}</div>
          <div style="font-size:11px; color:#888;">${htmlEscape(r.source_date || "")}</div>
        </div>
        <div style="padding:10px 16px;">
          <div style="font-size:13px; font-weight:600; color:#1a1a1a; margin-bottom:2px;">${htmlEscape(r.target_title || "—")}</div>
          <div style="font-size:12px; color:#555;">${htmlEscape(r.target_authors || "")}</div>
          <div style="font-size:11px; color:#888;">${htmlEscape(r.target_date || "")}</div>
        </div>
      </div>
 
      <!-- ── PASSAGES COTE A COTE ── -->
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; padding:14px 16px; border-bottom:1px solid #e0e0e0; background:#fafafa;">
        ${renderPassageBlock("Source", "#dbeeff", "#0c3b6e", "#5b9fd6", srcPassage)}
        ${renderPassageBlock("Cible",  "#daf0e6", "#0a3d26", "#3aab72", tgtPassage)}
      </div>
...
// ─── fonctions globales appelees par onclick ──────────────────────────────────
// (inline onclick est la solution la plus robuste contre les conflits CSS/JS du projet)
 
window.toggleEditor = function(aid) {
  const ed = document.getElementById(`ed-${aid}`);
  if (!ed) return;
  ed.style.display = ed.style.display === "none" ? "block" : "none";
};
 
window.saveAnnot = async function(aid) {
  const statusEl  = document.getElementById(`sel-${aid}`);
  ...
  try {
    ...
    // Mise a jour de la pill
    const pillEl = document.getElementById(`pill-${aid}`);
    if (pillEl) pillEl.innerHTML = statusPill(status);
 
    // Invalider le cache historique
    const histBtn = document.querySelector(`[data-hist-aid="${aid}"]`);
    if (histBtn) histBtn.__loaded = false;
 
    // Rafraichir l'historique s'il est visible
    ...
    msgEl.textContent = "Annotation enregistrée ✓";
    setTimeout(() => {
      const ed = document.getElementById(`ed-${aid}`);
...
window.toggleHist = async function(panelId, contentId, aid) {
  ...
  // Chargement a la premiere ouverture
  ...
  if (!isOpen && !window[cacheKey]) {
    ...
    if (contentEl) {
      try {
        ...
        contentEl.innerHTML = `<p style="font-size:12px; color:#c00;">${htmlEscape(String(e))}</p>`;
      }
    }
  }
};
...
const DEFAULT_STATUS_FILTERS = {
  OUI: true, NON: false, DOUTEUX: true, UNREVIEWED: true, DISCARDED: false,
};
 
function buildStatusCheckboxes(statusFilters) {
  const labels = { OUI: "Correct", NON: "Incorrect", DOUTEUX: "Douteux", UNREVIEWED: "Non-révisé", DISCARDED: "Ignoré" };
    ...
    <section class="align-search-card">
      <form id="alignSearchForm" class="align-search-form">
        <div class="align-quick-row">
          <input type="text" id="q" name="q" placeholder="Recherche rapide : auteur ou texte…" class="align-input align-input-main" />
...
function collectSearchParams(statusFilters) {
  ...
  for (const field of fields) {
    ...
    if (value !== "" && value != null) params.append(field, value);
  }
  const activeStatuses = Object.entries(statusFilters).filter(([,v]) => v).map(([s]) => s);
  if (activeStatuses.length === 1) params.append("status", activeStatuses[0]);
...
export async function renderAlignmentsPage(app) {
  ...
  function renderPageSkeleton() {
    app.innerHTML = `
      <div class="wrap alignment-page">
        <div style="margin-bottom:12px;"><a href="#/">← Retour</a></div>
  ...
  function bindEvents() {
    document.getElementById("alignSearchForm")?.addEventListener("submit", async e => {
      e.preventDefault(); await fetchAndRender();
    });
    document.getElementById("resetBtn")?.addEventListener("click", () => renderAlignmentsPage(app));
    document.getElementById("toggleAdvancedBtn")?.addEventListener("click", () => {
      showAdvanced = !showAdvanced; renderPageSkeleton(); bindEvents();
    });
    document.querySelectorAll(".js-status-filter").forEach(el => {
      el.addEventListener("change", e => { statusFilters[e.target.value] = e.target.checked; });
    });
  }
